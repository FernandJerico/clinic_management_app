...
class MedicalRecordsRemoteDatasource {
  ...
  Future<Either<String, GetMedicalRecordsResponseModel>>
      getMedicalRecordByScheduleId(int scheduleId) async {
    final authData = await AuthLocalDatasources().getAuthData();
    final url = Uri.parse(
        '${Variables.baseUrl}/api/api-medical-records?schedule_id=$scheduleId');
    final response = await http.get(
      url,
      headers: {
        'Authorization': 'Bearer ${authData?.token}',
    ...
    if (response.statusCode == 200) {
      ...
      return const Left('Failed to get medical records by schedule id');
    }
  }

  Future<Either<String, GetMedicalRecordsResponseModel>> getMedicalRecordByName(
      String name) async {
    final authData = await AuthLocalDatasources().getAuthData();
    final url =
        Uri.parse('${Variables.baseUrl}/api/api-medical-records?name=$name');
    final response = await http.get(
      url,
      headers: {
        'Authorization': 'Bearer ${authData?.token}',
